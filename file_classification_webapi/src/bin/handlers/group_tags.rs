...
use file_classification_core::service::group_tag::{
  delete_group_tags_by_conditions, delete_group_tags_by_dtos,
  select_group_tags_by_conditions_with_options, select_group_tags_by_conditions_with_pagination,
  select_group_tags_by_filter_with_limit, select_group_tags_by_filter_with_options,
  select_group_tags_by_filter_with_pagination,
};
...
async fn api_list_group_tags_by_filter_with_limit(
  ...
  match select_group_tags_by_filter_with_limit(&mut conn, filter, limit.into_inner()) {
    Ok(group_tags) => {
      ...
      // 构造成功的响应对象并返回
      Ok(HttpResponse::Ok().json(ApiResponse::success_with_count(group_tags, count)))
    }
...
async fn api_list_group_tags_by_conditions_with_limit(
  ...
  // 调用核心服务层的方法执行查询
  match select_group_tags_by_conditions_with_limit(
    &mut conn,
...
async fn api_create_group_tag(
  ...
  match create_group_tag(&mut conn, group_tag_dto.into_inner()) {
    ...
    {
      Ok(HttpResponse::Created().json(ApiResponse::success_with_msg((), "组标签关联创建成功")))
    }

    // 错误处理
    Err(e) => Ok(
      HttpResponse::InternalServerError()
        .json(ApiResponse::<()>::error_with_code("CREATE_GROUP_TAG_FAILED", &e.to_string())),
    ),
  }
}
...
async fn api_delete_group_tag(
  ...
  match delete_group_tag_by_dto(&mut conn, &group_tag_dto.into_inner()) {
    ...
    // 成功时返回 OK 状态码及确认消息
    {
      Ok(HttpResponse::Ok().json(ApiResponse::success_with_msg((), "组标签关联删除成功")))
    }

    // 错误处理
    Err(e) => Ok(
      HttpResponse::InternalServerError()
        .json(ApiResponse::<()>::error_with_code("DELETE_GROUP_TAG_FAILED", &e.to_string())),
    ),
  }
}
...
async fn api_list_group_tags_by_filter_with_pagination(
  ...
  let filter: GroupTagFilter = serde_json::from_str(&filter_str)
    .unwrap_or_else(|_| GroupTagFilter { group_id: None, tag_id: None });
  let options: file_classification_core::model::models::GroupTagQueryOptions =
...
async fn api_list_group_tags_by_conditions_with_pagination(
  ...
  let conditions: Vec<GroupTagCondition> =
