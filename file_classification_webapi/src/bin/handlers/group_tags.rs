...
async fn api_create_group_tag(
    ...
    match create_group_tag(&mut conn, group_tag_dto.into_inner()) {
        ...
        // 错误处理
        Err(e) => Ok(
            HttpResponse::InternalServerError().json(ApiResponse::<()>::error_with_code("CREATE_GROUP_TAG_FAILED", &e.to_string())),
        ),
    }
}
...
async fn api_delete_group_tag(
    ...
    match delete_group_tag_by_dto(&mut conn, &group_tag_dto.into_inner()) {
        ...
        // 错误处理
        Err(e) => Ok(
            HttpResponse::InternalServerError().json(ApiResponse::<()>::error_with_code("DELETE_GROUP_TAG_FAILED", &e.to_string())),
        ),
    }
}
