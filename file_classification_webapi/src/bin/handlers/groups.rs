...
use file_classification_core::service::groups::{
  delete_groups_by_conditions, delete_groups_by_ids, get_group_by_id, select_group_by_file_id,
  select_group_by_tag_id, select_groups_by_conditions_with_options,
  select_groups_by_conditions_with_pagination, select_groups_by_filter_with_options,
  select_groups_by_filter_with_pagination, update_group_by_id,
};
...
/// 根据条件搜索组
///
/// 根据提供的条件数组搜索组，可以指定返回记录数量上限
///
/// 请求路径: GET /api/groups/search/by-conditions-with-limit
#[get("/api/groups/search/by-conditions-with-limit")]
async fn api_list_groups_by_conditions_with_limit(
  ...
  // 调用服务层根据条件查询组
  match select_groups_by_conditions_with_limit(
    &mut conn,
...
async fn api_create_group(
  ...
  match create_group(&mut conn, &group_dto) {
    ...
    Err(e) => Ok(
      HttpResponse::InternalServerError()
        .json(ApiResponse::<()>::error_with_code("CREATE_GROUP_FAILED", &e.to_string())),
    ),
  }
}
...
async fn api_update_groups_by_conditions(
  ...
  match update_groups_by_conditions(&mut conn, conditions, update_dto) {
    ...
    // 构造成功响应，包含更新记录数
    {
      Ok(
        HttpResponse::Ok()
          .json(ApiResponse::success_with_msg(count, &format!("成功更新 {} 条记录", count))),
      )
    }
    // 错误处理
    Err(e) => Ok(
      HttpResponse::InternalServerError()
        .json(ApiResponse::<()>::error_with_code("UPDATE_GROUP_FAILED", &e.to_string())),
    ),
  }
}
...
async fn api_delete_group_by_id(
  ...
  match delete_group(&mut conn, group_id) {
    ...
    Err(e) => Ok(
      HttpResponse::InternalServerError()
        .json(ApiResponse::<()>::error_with_code("DELETE_GROUP_FAILED", &e.to_string())),
    ),
  }
}
...
async fn api_list_groups_by_filter_with_pagination(
  ...
  let options: file_classification_core::model::models::GroupQueryOptions =
...
async fn api_list_groups_by_conditions_with_pagination(
  ...
  let conditions: Vec<GroupCondition> =
