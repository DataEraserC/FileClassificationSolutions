...
use file_classification_core::utils::database::{
  ConnectionError, establish_connection, run_pending_migrations,
};
use log;
use std::env;
use std::error::Error;
...
impl AppConfig {
  pub fn load() -> Result<Self, Box<dyn Error>> {
    ...
    let bind_port =
      env::var("BIND_PORT").ok().and_then(|s| s.parse::<u16>().ok()).unwrap_or(8082u16);

    // 获取上传路径配置，默认为 "uploads"
    let upload_path = env::var("UPLOAD_PATH").unwrap_or_else(|_| "uploads".to_string());

    Ok(AppConfig { database_url, database_type, bind_address, bind_port, upload_path })
  }

  pub fn run_migrations(&self) -> Result<(), Box<dyn Error>> {
    let mut conn = establish_connection(&self.database_url, &self.database_type).map_err(|e| {
      log::error!("数据库连接失败: {}", e);
      e
    })?;
