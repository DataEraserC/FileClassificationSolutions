...
pub fn create_cors() -> Cors {
  // 从环境变量获取 CORS 配置
  let cors_enabled =
    env::var("CORS_ENABLED").ok().and_then(|s| s.parse::<bool>().ok()).unwrap_or(true); // 默认启用 CORS

  let cors_origin =
    env::var("CORS_ORIGIN").ok().unwrap_or_else(|| "http://localhost:8082".to_string());

  // 从环境变量获取端口，默认为 8082
  let port = env::var("BIND_PORT").ok().and_then(|s| s.parse::<u16>().ok()).unwrap_or(8082u16);
