...
use crate::model::models::{
  GroupOrderBy, GroupQueryOptions, OrderDirection, PaginationResult, UpdateGroupDTO,
};
...
pub fn insert_group(
  ...
  match conn {
    ...
    AnyConnection::Sqlite(sqlite) => {
      diesel::insert_into(groups::table).values(new_group).returning(groups::id).get_result(sqlite)
    }
    #[cfg(feature = "mysql")]
    // 对于 MySQL 连接，使用事务方式
    AnyConnection::Mysql(mysql) => {
      mysql.transaction(|mysql| {
        // 执行插入操作
        diesel::insert_into(groups::table).values(new_group).execute(mysql)?;
    ...
    AnyConnection::Postgresql(postgres) => diesel::insert_into(groups::table)
      .values(new_group)
      .returning(groups::id)
      .get_result(postgres),
  }
...
pub fn find_group_by_id(
  ...
  groups.select(Group::as_select()).filter(groups::id.eq(group_id)).first::<Group>(conn).optional()
}
...
pub fn delete_groups_by_ids(
  ...
  diesel::delete(groups::table.filter(groups::id.eq_any(group_ids))).execute(conn)
}
...
> {
  match condition {
    ...
    GroupCondition::DescriptionLike(pattern) => {
      Box::new(groups::description.like(pattern).assume_not_null())
    }
    ...
    GroupCondition::DescriptionIn(values) => {
      Box::new(groups::description.eq_any(values).assume_not_null())
    }
...
pub fn update_groups_by_conditions(
  ...
  let mut query =
    diesel::update(groups::table).into_boxed::<<AnyConnection as Connection>::Backend>();
...
pub fn get_group_by_id(
  ...
  groups::table.filter(groups::id.eq(group_id)).select(Group::as_select()).first(conn)
}
...
/// 返回值:
/// 成功时返回影响的行数（通常应为1），失败则返回数据库错误
pub fn update_group_parent_id(
  conn: &mut AnyConnection,
  child_of_relation: i32,
  parent_of_relation: i32,
) -> Result<usize, diesel::result::Error> {
