  ...
  match conn {
    ...
    AnyConnection::Mysql(mysql) => {
      mysql.transaction(|mysql| {
        // 执行插入操作
        diesel::insert_into(tags::table).values(new_tag).execute(mysql)?;
...
pub fn select_tags_by_filter_with_limit(
  ...
  let mut base_query = tags.into_boxed::<<AnyConnection as Connection>::Backend>();
...
> {
  match condition {
    ...
    TagCondition::DescriptionLike(pattern) => {
      Box::new(tags::description.like(pattern).assume_not_null())
    }
    ...
    TagCondition::DescriptionIn(values) => {
      Box::new(tags::description.eq_any(values).assume_not_null())
    }
