...
/// 处理简化命令（如ls, cd等）
pub fn handle_simplified_command(line: &str, conn: &mut AnyConnection, context: &mut Context) -> bool {
...
fn handle_file_action(
    ...
    match action {
        cli::FileActions::Create {
            ...
            let group_id = group_id.unwrap_or_else(|| get_input("请输入组 ID: ").parse().expect("无效的组 ID"));
        ...
        cli::FileActions::Delete { id } => {
            let file_id = id.or(context.selected_file_id).ok_or("错误：未提供文件 ID，也未在上下文中选中任何文件。")?;
        ...
        cli::FileActions::ListByConditions {
            conditions,
            order_by,
            limit,
            offset,
        ...
        cli::FileActions::UpdateByConditions {
            conditions,
            path,
            type_,
            reference_count,
...
fn handle_group_action(
    ...
    match action {
        ...
        cli::GroupActions::Delete { id } => {
            let group_id = id.or(context.selected_group_id).ok_or("错误：未提供组 ID，也未在上下文中选中任何组。")?;
...
fn handle_tag_action(
    ...
    match action {
        ...
        cli::TagActions::ListByConditions {
            conditions,
            order_by,
            limit,
            offset,
        ...
        cli::TagActions::UpdateByConditions {
            conditions,
            name,
            reference_count,
        } => {
            let conditions = if conditions.is_empty() {
                if let Some(selected_id) = context.selected_tag_id {
                    ...
                    return Err("没有活动的标签，请先运行 'tag list' 或 'tag list-by-conditions' 选择一个标签".into());
                }
            } else {
...
fn handle_file_group_action(
    ...
    match action {
        ...
        cli::FileGroupActions::Delete { file_id, group_id } => {
            let file_id = file_id.or(context.selected_file_id).unwrap_or_else(|| get_input("请输入文件 ID: ").parse().expect("无效的文件 ID"));
            let group_id = group_id.or(context.selected_group_id).unwrap_or_else(|| get_input("请输入组 ID: ").parse().expect("无效的组 ID"));
        ...
        cli::FileGroupActions::ListByConditions {
            conditions,
            order_by,
            limit,
            offset,
...
fn handle_group_tag_action(
    ...
    match action {
        cli::GroupTagActions::Create { group_id, tag_id } => {
            let group_id = group_id.or(context.selected_group_id).unwrap_or_else(|| get_input("请输入组 ID: ").parse().expect("无效的组 ID"));
            let tag_id = tag_id.unwrap_or_else(|| get_input("请输入标签 ID: ").parse().expect("无效的标签 ID"));
        ...
        cli::GroupTagActions::Delete { group_id, tag_id } => {
            let group_id = group_id.or(context.selected_group_id).unwrap_or_else(|| get_input("请输入组 ID: ").parse().expect("无效的组 ID"));
            let tag_id = tag_id.or(context.selected_tag_id).unwrap_or_else(|| get_input("请输入标签 ID: ").parse().expect("无效的标签 ID"));
        ...
        cli::GroupTagActions::ListByConditions {
            conditions,
            order_by,
            limit,
            offset,
